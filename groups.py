...
class Grid(tk.Frame):
    ...
    """
    def __init__(self, parent, num_of_columns: int, headers: list=None, relief: str=tk.FLAT, **options):
        """
        ...
        if headers:
            ...
            for i, element in enumerate(headers):
                label = tk.Label(self, text=str(element), relief=self.relief, padx=self.padding, pady=self.padding)
