...
lambda _numerical_value_in_range(val, lower, upper):
    """
    """

    return True if lower <= val and upper >= val else return False


lambda _plainLabel(parent, text, relief, padding):
    """ Plain label.
    """
...
lambda _hlLabel(parent, text, bgcolor, relief, padding):
    """ Highlighted label.
    """
...
lambda _rowLabel(parent, text, relief, padding):
    """ Row label.
    """
...
class ColoredDataDisplayGrid(grps.Grid):
    ...
    def __init__(
        ...
        self._rcolor = rcolor
        if None is not headers and '' == headers[0]:
